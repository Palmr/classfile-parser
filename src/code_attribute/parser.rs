...
pub fn instruction_parser(input: &[u8], address: usize) -> IResult<&[u8], Instruction> {
    switch!(input, be_u8,
        ...
        0xbd => map!(be_u16, |index| Instruction::Anewarray(index)) |
        0xb0 => value!(Instruction::Areturn) |
        0xbe => value!(Instruction::Arraylength) |
        0x3a => map!(be_u8, |index| Instruction::Astore(index)) |
        ...
        0x10 => map!(be_i8, |value| Instruction::Bipush(value)) |
        0x34 => value!(Instruction::Caload) |
        0x55 => value!(Instruction::Castore) |
        0xc0 => map!(be_u16, |index| Instruction::Checkcast(index)) |
        ...
        0x18 => map!(be_u8, |index| Instruction::Dload(index)) |
        ...
        0x39 => map!(be_u8, |index| Instruction::Dstore(index)) |
        ...
        0x17 => map!(be_u8, |index| Instruction::Fload(index)) |
        ...
        0x38 => map!(be_u8, |index| Instruction::Fstore(index)) |
        ...
        0xb4 => map!(be_u16, |index| Instruction::Getfield(index)) |
        0xb2 => map!(be_u16, |index| Instruction::Getstatic(index)) |
        0xa7 => map!(be_i16, |branch| Instruction::Goto(branch)) |
        0xc8 => map!(be_i32, |branch| Instruction::GotoW(branch)) |
        ...
        0xa5 => map!(be_i16, |branch| Instruction::IfAcmpeq(branch)) |
        0xa6 => map!(be_i16, |branch| Instruction::IfAcmpne(branch)) |
        0x9f => map!(be_i16, |branch| Instruction::IfIcmpeq(branch)) |
        0xa0 => map!(be_i16, |branch| Instruction::IfIcmpne(branch)) |
        0xa1 => map!(be_i16, |branch| Instruction::IfIcmplt(branch)) |
        0xa2 => map!(be_i16, |branch| Instruction::IfIcmpge(branch)) |
        0xa3 => map!(be_i16, |branch| Instruction::IfIcmpgt(branch)) |
        0xa4 => map!(be_i16, |branch| Instruction::IfIcmple(branch)) |
        0x99 => map!(be_i16, |branch| Instruction::Ifeq(branch)) |
        0x9a => map!(be_i16, |branch| Instruction::Ifne(branch)) |
        0x9b => map!(be_i16, |branch| Instruction::Iflt(branch)) |
        0x9c => map!(be_i16, |branch| Instruction::Ifge(branch)) |
        0x9d => map!(be_i16, |branch| Instruction::Ifgt(branch)) |
        0x9e => map!(be_i16, |branch| Instruction::Ifle(branch)) |
        0xc7 => map!(be_i16, |branch| Instruction::Ifnonnull(branch)) |
        0xc6 => map!(be_i16, |branch| Instruction::Ifnull(branch)) |
        0x84 => do_parse!(index: be_u8 >> value: be_i8 >> (Instruction::Iinc{index: index, value: value})) |
        0x15 => map!(be_u8, |index| Instruction::Iload(index)) |
        ...
        0xc1 => map!(be_u16, |index| Instruction::Instanceof(index)) |
        ...
        0xb7 => map!(be_u16, |index| Instruction::Invokespecial(index)) |
        0xb8 => map!(be_u16, |index| Instruction::Invokestatic(index)) |
        0xb6 => map!(be_u16, |index| Instruction::Invokevirtual(index)) |
        ...
        0x36 => map!(be_u8, |index| Instruction::Istore(index)) |
        ...
        0xa8 => map!(be_i16, |branch| Instruction::Jsr(branch)) |
        0xc9 => map!(be_i32, |branch| Instruction::JsrW(branch)) |
        ...
        0x12 => map!(be_u8, |index| Instruction::Ldc(index)) |
        0x13 => map!(be_u16, |index| Instruction::LdcW(index)) |
        0x14 => map!(be_u16, |index| Instruction::Ldc2W(index)) |
        0x6d => value!(Instruction::Ldiv) |
        0x16 => map!(be_u8, |index| Instruction::Lload(index)) |
        ...
        0x37 => map!(be_u8, |index| Instruction::Lstore(index)) |
        ...
        0xbb => map!(be_u16, |index| Instruction::New(index)) |
        0xbc => map!(be_u8, |atype| Instruction::Newarray(atype)) |
        0x00 => value!(Instruction::Nop) |
        0x57 => value!(Instruction::Pop) |
        0x58 => value!(Instruction::Pop2) |
        0xb5 => map!(be_u16, |index| Instruction::Putfield(index)) |
        0xb3 => map!(be_u16, |index| Instruction::Putstatic(index)) |
        0xa9 => map!(be_u8, |index| Instruction::Ret(index)) |
        ...
        0xc4 => switch!(be_u8,
            0x19 => map!(be_u16, |index| Instruction::AloadWide(index)) |
            0x3a => map!(be_u16, |index| Instruction::AstoreWide(index)) |
            0x18 => map!(be_u16, |index| Instruction::DloadWide(index)) |
            0x39 => map!(be_u16, |index| Instruction::DstoreWide(index)) |
            0x17 => map!(be_u16, |index| Instruction::FloadWide(index)) |
            0x38 => map!(be_u16, |index| Instruction::FstoreWide(index)) |
            0x15 => map!(be_u16, |index| Instruction::IloadWide(index)) |
            0x36 => map!(be_u16, |index| Instruction::IstoreWide(index)) |
            0x16 => map!(be_u16, |index| Instruction::LloadWide(index)) |
            0x37 => map!(be_u16, |index| Instruction::LstoreWide(index)) |
            0xa9 => map!(be_u16, |index| Instruction::RetWide(index)) |
